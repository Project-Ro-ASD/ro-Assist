...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), updateProcess(new QProcess(this)), checkUpdateProcess(new QProcess(this)), checkLibProcess(new QProcess(this)),
      carousel(new QStackedWidget(this)), carouselTimer(new QTimer(this)),
      logConsole(new QTextEdit(this)), libraryLogConsole(new QTextEdit(this)),
      transactionPhaseStarted(false), isUpToDate(false), isTerminatingIntentionally(false), isNetworkConnected(true),
      isLibraryInstalled(false), accentColor(QColor("#0066cc"))
{
    ...
    connect(checkLibProcess, &QProcess::finished, this, &MainWindow::handleCheckLibFinished);
...
void MainWindow::setupUi()
{
    ...
    connect(backFromLibraryBtn, &QPushButton::clicked, this, &MainWindow::showCarouselScreen);
    ...
    connect(libraryInstallButton, &QPushButton::clicked, this, &MainWindow::startLibraryPackageInstall);
    ...
    connect(toggleLibraryLogBtn, &QPushButton::clicked, this, &MainWindow::toggleLibraryLogs);
...
void MainWindow::createCarouselSlides()
{
    ...
    connect(libraryPackageSlideBtn, &QPushButton::clicked, this, &MainWindow::showLibraryScreen);
    l5->addStretch(); l5->addWidget(slide5Title); l5->addSpacing(20); l5->addWidget(slide5Desc); l5->addSpacing(40); l5->addWidget(libraryPackageSlideBtn, 0, Qt::AlignCenter); l5->addStretch();
...
void MainWindow::updateUiTextAndImages()
{
    ...
    if (isTr) {
        ...
        toggleLibraryLogBtn->setText(libraryLogConsole->isVisible() ? "Gizle ⬆" : "Logları Göster ⬇");
        ...
        slide5Desc->setText("Oyun kütüphanelerini indirebilir ve güncelleyebilirsiniz.");
        ...
        toggleLibraryLogBtn->setText(libraryLogConsole->isVisible() ? "Ocultar ⬆" : "Mostrar Registros ⬇");
        ...
        slide5Desc->setText("Puede descargar y actualizar bibliotecas para juegos.");
        ...
        toggleLibraryLogBtn->setText(libraryLogConsole->isVisible() ? "Hide ⬆" : "Show Logs ⬇");
    ...
    libraryLogConsole->setPlaceholderText(currentLang == TR ? "Log kayıtları / Logs..." : (currentLang == ES ? "Registros / Logs..." : "Logs..."));
    roAsdGitHubBtn->setText("ro-ASD OS\nRepo");
    roAssistGitHubBtn->setText("ro-Assist\nRepo");
    
    websiteBtn->setText(currentLang == TR ? "Web Sitesi" : (currentLang == ES ? "Sitio Web" : "Website"));
    if (isLibraryInstalled) {
        libraryInstallButton->setText(currentLang == TR ? "Kütüphaneleri Güncelle" : (currentLang == ES ? "Actualizar bibliotecas" : "Update Libraries"));
    } else {
        libraryInstallButton->setText(currentLang == TR ? "Kütüphaneleri İndir" : (currentLang == ES ? "Descargar bibliotecas" : "Download Libraries"));
    }
...
void MainWindow::setupStyle()
{
    ...
    QString btnHover = currentTheme == Dark ? "#3f3f46" : "#f1f5f9";
    QString inputBg = currentTheme == Dark ? "#18181b" : "#f1f5f9";

    QString accent = accentColor.name();
    QString accentDark = accentColor.darker(115).name();
    QString accentHover = accentColor.lighter(115).name();
    ...
    QString style = QString(R"(
        * { font-family: 'Segoe UI', 'Inter', 'Roboto', sans-serif; outline: none; }
        ...
        QLabel#slideTitle { font-size: 34px; font-weight: 800; margin-bottom: 8px; color: %7; }
        ...
        QToolButton#squareSoftButton { 
            ...
            background-color: %10; border: 1px solid %5; color: %3; padding: 12px; 
        }
        QToolButton#squareSoftButton:hover { background-color: %2; border: 1px solid %7; color: %7; }
      ...
      .arg(accent, accentDark, accentHover, inputBg);
...
{ 
    ...
    libraryInstallButton->setText(currentLang == TR ? "Kontrol ediliyor..." : (currentLang == ES ? "Comprobando..." : "Checking..."));
    
    if (checkLibProcess->state() == QProcess::NotRunning) {
        checkLibProcess->start("rpm", QStringList() << "-q" << "gamemode" << "mangohud" << "vulkan-loader" << "vulkan-tools");
    }
}
...
libraryInstallButton->setEnabled(false);
    ...
    libraryStatusLabel->setText(currentLang == TR ? "Kütüphaneler kuruluyor..." : (currentLang == ES ? "Instalando paquetes..." : "Installing packages..."));
    
    libraryLogConsole->clear();
    appendLog(currentLang == TR ? "Kütüphaneler kuruluyor..." : (currentLang == ES ? "Instalando bibliotecas..." : "Installing libraries..."), "#0066cc");
    ...
    updateProcess->start("pkexec", QStringList() << "dnf" << action << "-y" << "gamemode" << "mangohud" << "vulkan-loader" << "vulkan-tools");
}
...
void MainWindow::checkDnfErrors(const QString &output)
{
    ...
    if (output.contains("Waiting for process", Qt::CaseInsensitive) || output.contains("Another app is currently holding the yum lock", Qt::CaseInsensitive)) {
        ...
        if (actIdx == 2) libraryStatusLabel->setText(msg);
...
void MainWindow::handleUpdateErrorOutput()
{
    ...
    if (errorOutput.contains("standard input:1", Qt::CaseInsensitive) || errorOutput.contains("incorrect password", Qt::CaseInsensitive) || errorOutput.contains("try again", Qt::CaseInsensitive)) {
        ...
        if (actIdx == 2) {
            ...
            libraryStatusLabel->setText(t);
            libraryInstallButton->setEnabled(true);
            if (!libraryLogConsole->isVisible()) toggleLibraryLogs();
        } else {
            progressBar->hide();
            progressBar->setValue(0);
            statusLabel->setText(t);
...
void MainWindow::handleCheckLibFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
...
void MainWindow::handleUpdateFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (actIdx == 2) {
        ...
        if (exitStatus == QProcess::NormalExit && exitCode == 0) {
            ...
            libraryStatusLabel->setText(currentLang == TR ? "✅ İşlem Başarıyla Tamamlandı!" : (currentLang == ES ? "✅ ¡Completado con Éxito!" : "✅ Process Completed!"));
            appendLog(currentLang == TR ? "Kütüphaneler kuruldu." : (currentLang == ES ? "Bibliotecas instaladas." : "Libraries installed."), "#00cc00");
        } else {
            if (!isTerminatingIntentionally) {
                libraryProgressBar->hide();
                libraryStatusLabel->setText(currentLang == TR ? "İşlem Başarısız!" : (currentLang == ES ? "¡Proceso fallido!" : "Process Failed!"));
...
void MainWindow::handleUpdateProcessError(QProcess::ProcessError error)
{
    ...
    if (actIdx == 2) {
        libraryProgressBar->hide();
        libraryInstallButton->setEnabled(true);
        libraryStatusLabel->setText((currentLang == TR ? "Kritik Hata: " : (currentLang == ES ? "Error crítico: " : "Critical Error: ")) + errorMsg);
        if (!libraryLogConsole->isVisible()) toggleLibraryLogs();
    } else {
        progressBar->hide();
        updateButton->setEnabled(true);
